...
検証には原則として `/doc/verify` 配下のWindows 11環境（推奨：`/doc/verify/windows-11-22H2`）およびWindows 10環境（推奨：`/doc/verify/windows-10-21H2`）を使用する。
...
7. Edgeのバージョン情報を表示し、自動更新機能でEdgeを最新版に更新する。

## 検証

### インストーラー検証

#### タスクスケジューラー設定（Windows 10、Windows 11）
...
### BHO無効時のIEモードのタブの挙動を含む、Manifest V3での動作（Windows 11）
...
### BHO有効時のIEモードのタブの挙動を含む、Manifest V3での動作（Windows 10、Windows 11）
...
### EdgeのIEモード境界をまたぐページ遷移における設定無効時の動作（Windows 10、Windows 11）
...
### EdgeのIEモード境界をまたぐページ遷移におけるDefault設定の動作（Windows 10、Windows 11）
...
3. IEのみに該当するURLの動作の検証：
   ...
   2. Edgeで新しいタブで https://groonga.org/related-projects.html を開く。
      * 期待される結果：
        ...
        * [ ] ThinBridgeによるリダイレクトが発生しない。
   3. `Groonga-GObject` のリンクをクリックする。
      * 期待される結果：
        * [ ] タブがページ遷移しない。
        * [ ] ThinBridgeによるリダイレクトが発生し、 https://github.com/groonga/groonga-gobject がIEで開かれるか、IEの起動を試みている旨を示すダイアログが表示される。
   ...
   8. Edgeで新しいタブで https://groonga.org/related-projects.html を開く。
      * 期待される結果：
        ...
        * [ ] ThinBridgeによるリダイレクトが発生しない。
   9. アドレスバーに https://github.com/groonga/groonga-gobject を入力し、Enterする。
      * 期待される結果：
   ...
   11. Edgeで新しいタブで https://groonga.org/related-projects.html を開く。
       * 期待される結果：
         * [ ] EdgeのIEモードのタブで https://groonga.org/related-projects.html が読み込まれる。
         * [ ] ThinBridgeによるリダイレクトが発生しない。
   12. `Groonga-GObject` のリンクをクリックする。
       * 期待される結果：
          * [ ] Edgeのタブがページ遷移しない。
          * [ ] ThinBridgeによるリダイレクトが発生し、 https://github.com/groonga/groonga-gobject がIEで開かれるか、IEの起動を試みている旨を示すダイアログが表示される。
          * [ ] (Windows Enterprise マルチセッションのみ) ユーザーBでIEへのリダイレクトが発生しない。
4. 共用URL（CUSTOM18）に該当するURLの動作の検証：
   ...
   3. `Rroonga` のリンクをクリックする。
      * 期待される結果：
        * [ ] Edgeの通常タブで https://ranguba.org/#about-rroonga が開かれる。
        * [ ] ThinBridgeによるリダイレクトが発生しない。
   4. Edge（通常モード）→Edge（IEモード）の検証のため、`edge://settings/defaultBrowser` を開き、IEモードの対象URLを以下の通り設定する。
      * 追加： https://ranguba.org/#about-rroonga
   5. Edgeで新しいタブで https://groonga.org/related-projects.html を開く。
      * 期待される結果：
        * [ ] Edgeの通常タブで https://groonga.org/related-projects.html が読み込まれる。
        * [ ] ThinBridgeによるリダイレクトが発生しない。
   6. `Rroonga` のリンクをクリックする。
      * 期待される結果：
        * [ ] タブがIEモードに切り替わり、 https://ranguba.org/#about-rroonga が開かれる。
        * [ ] ThinBridgeによるリダイレクトが発生しない。
   7. Edge（IEモード）→Edge（通常モード）の検証のため、`edge://settings/defaultBrowser` を開き、IEモードの対象URLを以下の通り設定する。
      * 削除： https://ranguba.org/#about-rroonga
      * 追加： https://groonga.org/related-projects.html
   8. Edgeで新しいタブで https://groonga.org/related-projects.html を開く。
      * 期待される結果：
        * [ ] EdgeのIEモードのタブで https://groonga.org/related-projects.html が読み込まれる。
        * [ ] ThinBridgeによるリダイレクトが発生しない。
   9. アドレスバーに https://ranguba.org/#about-rroonga を入力し、Enterする。
      * 期待される結果：
        * [ ] タブが通常モードに切り替わり、 https://ranguba.org/#about-rroonga が開かれる。
        * [ ] ThinBridgeによるリダイレクトが発生しない。
   10. Edge（IEモード）→Edge（IEモード）の検証のため、`edge://settings/defaultBrowser` を開き、IEモードの対象URLを以下の通り設定する。
       * 追加： https://ranguba.org/#about-rroonga
   11. Edgeで新しいタブで https://groonga.org/related-projects.html を開く。
   ...
   12. `Rroonga` のリンクをクリックする。
       * 期待される結果：
         * [ ] EdgeのIEモードのタブで https://ranguba.org/#about-rroonga が読み込まれる。
         * [ ] ThinBridgeによるリダイレクトが発生しない。
...
### EdgeのIEモード境界をまたぐページ遷移におけるDefault設定なしの動作（Windows 10、Windows 11）
...
### `onTabUpdated()`によるリダイレクトの動作検証（Windows 11）
...
### サブフレームでのリダイレクトの動作検証（Windows 11）
...
1. Edgeを起動する。
2. 新規タブを開く。
3. URLエントリに `https://www.fluentd.org/videos/` を入力してEnterキーを押下する。
   * 期待される結果
     * [ ] トップフレームは https://www.fluentd.org/videos/ に遷移する。
...
### アドオンインストール前から存在する既存タブでの挙動（Windows 11）
...
### 既定のブラウザによるリダイレクト対象URLの挙動（Windows 11）
