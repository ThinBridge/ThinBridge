...
5. Microsoftストアへの登録申請
   ...
   * アカウントは `edge@clear-code.com` を使用し、パスキーでログインする
   ...
   2. [Microsoftストアのダッシュボード](https://partner.microsoft.com/en-us/dashboard/microsoftedge/overview)で項目を選択する。
      * 対象項目名が **ThinBridge** であることを指差し確認する。
      * **ThinBridge Enterprise Developer Edition** は **ThinBridge** の更新でトラブルが起こった際の切り戻し先として運用しているため、新バージョンのリリース時には触らない。
   3. 「Packages」を選択する。
   4. 既存パッケージの「Replace or Drag your package here (.zip)」にファイルをドラッグ＆ドロップで登録する。
      * ファイル名が **ThinBridgeEdge.zip** であることを指差し確認する。
   5. 「Continue」をクリックする。
...
6. Chromeストアへの登録申請
   ...
   * アカウントは `google@clear-code.com` を使用し、パスキーでログインする
   ...
   2. Chrome Webストアのダッシュボードを開き、項目を選択する。
      * 対象アイテム名が **ThinBridge** であることを指差し確認する。
      * **ThinBridge Enterprise Developer Edition** は **ThinBridge** の更新でトラブルが起こった際の切り戻し先として運用しているため、新バージョンのリリース時には触らない。
   3. 「パッケージ」を選択する。
   4. 「新しいパッケージをアップロード」をクリックする。
   5. ファイルをドラッグ＆ドロップで登録する。
      * ファイル名が **ThinBridgeChrome.zip** であることを指差し確認する。
7. ストアでの公開後の対応
