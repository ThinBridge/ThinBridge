...
以下、リリースするバージョンを4.2.1.0と仮定して説明する。
 ...
 1. masterブランチを使用して[リリース前検証](doc/verify/PreReleaseVerification.md)を一通り実施し、不具合がないことを確認する。
    ```console
    > git switch master
    ```
...
1. [リリース前検証手順](doc/verify/PreReleaseVerification.md)に従って、リリース前の動作確認を行う。
   * 必須の確認対象はEdgeである。
2. GitHubリリースの作成
